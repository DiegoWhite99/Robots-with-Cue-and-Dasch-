    ...
    async def scan_and_connect(self):
        ...
        try:
            ...
            def sync_sendJson(json_dict):
                ...
                loop = self.bleak_loop
                if loop is None or loop.is_closed():
                    return
                coro = self.sendJson(json_dict)
                try:
                    asyncio.run_coroutine_threadsafe(coro, loop)
                except Exception:
                    # Si el loop se cierra mientras llegan sensores, descartamos
                    # silenciosamente comandos tardíos de cierre.
                    try:
                        coro.close()
                    except Exception:
                        pass
            ...
            await asyncio.sleep(0.5)   # dejar asentar el enlace

            # NO renegociamos el intervalo de conexion: el valor por defecto (~30ms) es
            # mas ESTABLE en Windows. El intervalo agresivo (12ms) causaba desconexiones
            # con el Dash. (Reactivar solo si necesitas baja latencia en otra plataforma.)
            ...
            # Suscripcion a sensores: UN intento, best-effort. Si falla, seguimos igual:
            # mover/luces NO necesitan sensores, y reintentar hammerea el enlace inestable
            # (el Dash se desconectaba). Mejor un enlace estable para los comandos.
            async def _try_notify(char_uuid):
                try:
                    await self.client.start_notify(char_uuid, on_data_sensor)
                    return True
                except Exception as e:
                    print('[WonderPy] sensores %s no disponibles (se ignora): %s' % (char_uuid, e))
                    return False

            await _try_notify(CHAR_UUID_SENSOR0)
            _sensor1_ok = False
            if self.robot.expect_sensor_packet_2:
                _sensor1_ok = await _try_notify(CHAR_UUID_SENSOR1)
