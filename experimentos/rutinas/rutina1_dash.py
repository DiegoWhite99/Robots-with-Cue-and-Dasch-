" rutina uno de movimiento en el pasillo del piso 3 — Dash robot."
...
class Rutina1Dash:
    ...
    def _luz(self, robot, color):
        ...
        robot.cmds.RGB.stage_front(r, g, b)
        robot.send_staged()
    ...
    def _parpadear(self, robot, color, veces=3):
        ...
        for _ in range(veces):
            ...
            robot.send_staged()
            time.sleep(0.2)
            robot.cmds.RGB.stage_ear_left(*COLOR_APAGADO)
            robot.cmds.RGB.stage_ear_right(*COLOR_APAGADO)
            robot.cmds.RGB.stage_front(*COLOR_APAGADO)
            robot.send_staged()
            time.sleep(0.2)
    ...
    def _esperar_libre(self, robot):
        while self._hay_obstaculo(robot):
            if not self._obstaculo:
                ...
                print("!! Obstacle detected — waiting for clear path...")
        ...
        if self._obstaculo:
            self._obstaculo = False
            print(">> Path clear — resuming route")
    ...
    def on_connect(self, robot):
        print(f"Connected: {robot.name} ({robot.robot_type_name})")
        hablar("Dash robot connected. Starting routine.")
...
if __name__ == '__main__':
    ...
    wwMain.start(Rutina1Dash(), parser.parse_args(['--connect-eager']))
